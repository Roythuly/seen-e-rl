...
def _experiment_config(algorithm: str, tmp_path: Path) -> dict:
    ...
    return {
        "run_name": f"{algorithm}-contract-test",
        "seed": 13,
        ...
        "buffer": buffer,
        "eval": {"selector": "latest", "seeds": [17], "episodes_per_seed": 1},
        "artifacts": {"root": str(tmp_path / algorithm)},
    }


def test_runtime_artifacts_match_contracts(tmp_path: Path) -> None:
    for algorithm, builder in (
        ...
        validate_contract_payload("policy_snapshot.schema.json", train_result["published_policy"])
        for checkpoint_manifest in train_result["checkpoints"]:
            validate_contract_payload("checkpoint_manifest.schema.json", checkpoint_manifest)
